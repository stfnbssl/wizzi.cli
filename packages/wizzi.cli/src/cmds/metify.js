...
var file = vfile();
module.exports = {
    simple: (sourceFolder, outputPath) => {
    
        console.log('metify.simple.sourceFolder', sourceFolder, __filename);
        console.log('metify.simple.outputPath', outputPath, __filename);
    ...
    , 
    complex: (sourceFolder, outputPath) => {
    
        console.log('metify.complex.sourceFolder', sourceFolder, __filename);
        console.log('metify.complex.outputPath', outputPath, __filename);
        var sb1 = [];
        file.getFiles(sourceFolder, {
            deep: true, 
            documentContent: true
            ...
            for (var k in afs.folders) {
                ...
                for (var ke in folder) {
                    ...
                    for (i=0; i<i_len; i++) {
                        ...
                        file.write(path.join(outputPath, 'ittfDocumentTemplates', tfolder(k), af.basenameNoExt + '.' + ke + '.ittf.ittf'), af.content)
                    }
                    sb1.push('\t\t];');
                    sb1.push('');
                    sb1.push('\t$foreach item in items');
                    sb1.push('\t\t$file __dot__wizzi/<folder>/' + k + '/$' + '{item}' + '.' + ke + '.ittf.ittf');
                    if (af.firstWord) {
                    ...
                    sb1.push('\t\t\t\t$' + "{'$'}" + 'include <productionFolder>/' + tfolder(k) + '/$' + '{item}' + '.' + ke);
                }
            }
            file.write(path.join(outputPath, 'folderTemplates', 'index.ittf.ittf'), sb1.join('\n'))
        }
        )
    }
