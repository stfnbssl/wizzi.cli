...
class commandChecker {
    ...
    checkNotEmpty(name, kind, options) {
        ...
        if (verify.isEmpty(name)) {
            return this.optionError(verify.capitalize(kind) + ' ' + options.message + ': missing parameter');
        }
        this[kind] = name;
        return this.usingOption(verify.capitalize(kind) + ': ' + name);
    }
    checkArrayNotEmpty(items, kind, options) {
        ...
        if (!verify.isArray(items)) {
            return this.optionError(verify.capitalize(kind) + ' ' + options.message + ': is not an array');
        }
        if (items.length < 1) {
            return this.optionError(verify.capitalize(kind) + ' ' + options.message + ': the array is empty');
        }
        this[kind] = items;
        return this.usingOption(verify.capitalize(kind) + ': ' + items.length + ' items');
    }
    checkFile(fileName, kind, options) {
        kind = kind || '__';
        options = options || { message: '' };
        if (verify.isEmpty(fileName)) {
            return this.optionError(verify.capitalize(kind) + ' ' + options.message + ': missing file name');
        }
        ...
        if (!file.exists(filePath)) {
            if (options.parentFolderMustExist) {
                if (!file.exists(path.dirname(filePath))) {
                    return this.optionError(verify.capitalize(kind) + ' ' + options.message + ' parent folder must exist: ' + filePath);
                }
            }
            return this.optionError(verify.capitalize(kind) + ' ' + options.message + ' file not found: ' + filePath);
        }
        this[kind] = filePath;
        this[kind+'_is_folder'] = file.isDirectory(filePath);
        return this.usingOption(verify.capitalize(kind) + ': ' + filePath);
    }
    checkFolder(folderName, kind) {
        if (verify.isEmpty(folderName)) {
            return this.optionError(verify.capitalize(kind) + ': missing folder name');
        }
        ...
        if (!file.exists(folderPath)) {
            return this.optionError(verify.capitalize(kind) + ' ' +  'folder not found: ' + folderName);
        }
        this[kind] = folderPath;
        return this.usingOption(verify.capitalize(kind) + ': ' + folderPath);
    }
    isValid() {
